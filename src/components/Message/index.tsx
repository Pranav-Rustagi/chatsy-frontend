...
const MessageBox: React.FC<MessageBoxProps> = ({ message, sender, is_group = false, timestamp, type }: MessageBoxProps) => {
    return (
        <div className={`flex ${type === 'from' ? 'flex-row' : 'flex-row-reverse'} mx-5`}>
            <div className={`relative shadow-sm p-3 max-w-[60%] rounded-lg flex flex-col ${type === 'from' ? 'from-message-container bg-chatsy-light text-chatsy-text-dark rounded-tl-none' : 'to-message-container bg-chatsy-orange text-white rounded-tr-none'}`}>
                {
                    is_group && type === 'from' ?
